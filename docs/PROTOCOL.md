...
> **范围说明（2026-09-21）**：本协议仅用于解释方向调整前的 tcbench 历史 run，协议语义本身未改。它不是当前 SCoRe 忠实复现协议，也不再定义待执行的旧方法 sweep。当前工作使用 `docs/SCORE_REPRO_PROTOCOL.md`；不要依据本文的 Phase 1 描述自动启动实验。
